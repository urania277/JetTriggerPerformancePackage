...
## input files
fin1 = TFile.Open("Turnons_ChristiansRootFiles/r29926_periodJ.root")
fin2 = TFile.Open("hist-20160425_PeriodJ_282625_r7734_WithOldCuts_eta32/hist-20160425_PeriodJ_282625_r7734.root")

#fin1 = TFile.Open("Turnons_ChristiansRootFiles/ChristianTurnonsPeriodJ.root")
...
# additional label
label = "Edgars old cuts"
#label = "Christians cuts"
...
turnOnData = open("logfile_turnOns_CompChristian.txt") # newest list of JTPP
...
for multiHistoName in multiHistoList:
  ...
  ## Legend
  legend = TLegend(0.5,0.40,0.9,0.2)
  legend.SetFillColor(0)
  legend.SetTextFont(1)
  legend.SetHeader("2015 Period J")
  ...
  for turnOn in turnOnList:
    ...
    for EvalType in ["Emu"]: # ["Emu", "TDT"]:
      ...
      if (turnOn == "L1_J15-HLT_j15"):
        for n in range (0, 13):
          histo2.Rebin(3)
          histo2.SetBinContent(n, -1)
          ErrGraph2.SetPoint(n, 1000, 1000)

      if (turnOn == "L1_J40-HLT_j25"):
        for n in range (0, 13):
          histo2.Rebin(4)
          histo2.SetBinContent(n, -1)
          ErrGraph2.SetPoint(n, 1000, 1000)
    ...
    # Set Title
    l = TLatex(20,1.11, "#font[12]{" + label + "}")
    l.Draw("Same")
